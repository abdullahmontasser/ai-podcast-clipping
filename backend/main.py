...
def patch_numpy_issue():
    """
    Hotfix for the 'AttributeError: module numpy has no attribute int'
    in the s3fd library.
    """
    target_file = "/asd/model/faceDetector/s3fd/box_utils.py"

    try:
        with open(target_file, "r") as f:
            content = f.read()

        # This is the specific line causing the crash
        # Old: return np.array(keep).astype(np.int)
        # New: return np.array(keep).astype(int)

        if "np.int)" in content:
            print(f"Patching {target_file}...")
            new_content = content.replace("np.int)", "int)")

            with open(target_file, "w") as f:
                f.write(new_content)
            print("Patch successful.")
        else:
            print("File already patched or target string not found.")
...
def process_clip(
    ...
    try:
        ...
        print("Columbia script crashed! Check logs above.")
        raise e
    ...
    s3_client.upload_file(
        subtitle_output_path, "ai-podcast-clipper-seniors", output_s3_key
    )
...
class AiPodcastClipper:
    ...
    def load_model(self):
        ...
        self.whisperx_model = whisperx.load_model(
            ...
            vad_options={"vad_onset": 0.8, "vad_offset": 0.5},
        )
    ...
    def process_video(
        ...
        s3_client = boto3.client("s3")
        s3_client.download_file("ai-podcast-clipper-seniors", s3_key, str(video_path))
...
def main():
    ...
    payload = {"s3_key": "test1/mi630min.mp4"}
